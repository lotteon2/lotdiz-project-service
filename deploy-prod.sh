...
if ! kubectl get deployment project-deployment &> /dev/null; then
  echo "Create new kubernetes deployment resources..."
  kubectl create -f ./deployment-prod.yml
else
  echo "Apply new docker image "
  kubectl set image deployments/project-deployment project-service=$ECR_REGISTRY/$AWS_ECR_REPOSITORY:$IMAGE_TAG --record
fi
...
if ! kubectl get deployment project-deployment &> /dev/null; then
  ...
  if ! kubectl rollout status deployment/project-deployment --watch=false | grep -q "successfully rolled out"; then
    echo "Rolling update has failed. Initiating rollback..."

    kubectl rollout undo deployment/project-deployment
    kubectl rollout status deployment/project-deployment --watch=true
    ...
    if ! kubectl get service project-service &> /dev/null; then
